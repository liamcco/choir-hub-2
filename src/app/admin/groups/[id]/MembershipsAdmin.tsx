...
function AddMembershipPanel({
  ...
  const directMemberUserIds = new Set(memberships.filter((m) => m.isDirect).map((membership) => membership.userId))
  const availableUsers = users.filter((user) => !directMemberUserIds.has(user.id))
  const form = useForm({
    defaultValues: {
      userId: '',
    },
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {members.filter((x) => x.isDirect).length} direct / {members.length} effective
        </CardDescription>
        <CardAction>
          <Button
            type="button"
