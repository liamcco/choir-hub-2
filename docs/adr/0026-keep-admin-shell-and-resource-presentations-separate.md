...
The User collection uses explicit client-side grouping modes after transient search filtering: Name is a flat name sort, Status and Voice Type render expanded groups, and Home Choir renders nested Choir → Voice Type groups, all with deterministic domain ordering and name ordering within the innermost group. Voice Type values are the fine-grained `S1` through `B2` values; base voice values are not valid user placement values.
