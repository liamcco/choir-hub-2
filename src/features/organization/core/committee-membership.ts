...
import { group, groupMembership } from '@/drizzle/schema'
...
export const committeeMembership = {
  ...
  async start(input: { userId: string; groupId: string; startsAt?: Date; endsAt?: Date | null }) {
    const [target] = await db.select().from(group).where(eq(group.id, input.groupId)).limit(1)
    if (!target) throw new EntityDoesNotExistError('Choose an existing Group.')
